...
def process_output_images(outputs, job_id):
    ...
    for node_id, output_image in local_images.items():
        print(f"runpod-worker-comfy - output image {node_id}: {output_image}")
        ...
        if os.path.exists(local_image_path):
            if os.environ.get("BUCKET_ENDPOINT_URL", False):
                ...
                output_images[node_id] = rp_upload.upload_image(job_id, local_image_path)
                print("runpod-worker-comfy - the image was generated and uploaded to AWS S3")
            else:
                # base64 image
                output_images[node_id] = base64_encode(local_image_path)
